...
Round2Enemy[] round2enemies = new Round2Enemy[8];
Round2Enemy[] round2enemies2 = new Round2Enemy[7];
Round2Enemy[] round2enemies3 = new Round2Enemy[8];
///enemies in round 2
...
String text2 = "Stay alert, here comes the next wave!";
String text3 = "Nice shooting! We're in the clear.";
String text4 = "Time to settle in for another nap...";
PFont font;
int counter;
int counter2;
int counter3;
...
music = new SoundFile(this,"Go_Gadget_-_04_-_Password.wav");
...
round2enemies[i] = new Round2Enemy(color( 150, 10, 255), 1300, 75*i+75, 1.3);
}
///////////////////////////////////////////////////////////
for(int i =0; i < round2enemies2.length; i++) {
round2enemies2[i] = new Round2Enemy(color( 100, 255, 255), 1200, 75*i+75, 1.3);
}
//////////////////////////////////////////////////////////
for(int i =0; i < round2enemies3.length; i++) {
round2enemies3[i] = new Round2Enemy(color( 255, 100, 255), 1100, 75*i+75, 1.3);
}
...
textSize(30);
 ...
 for (int i = 0; i < round2enemies.length; i++) {
 round2enemies[i].display();
 round2enemies[i].move();
 round2enemies[i].bulletcontact();
}
...
for (int i = 0; i < round2enemies2.length; i++) {
 round2enemies2[i].display();
 round2enemies2[i].move();
 //enemies2[i].keyPressed();
 round2enemies2[i].bulletcontact();
}
//calling up and animating array

for (int i = 0; i < round2enemies3.length; i++) {
 round2enemies3[i].display();
 round2enemies3[i].move();
 round2enemies3[i].bulletcontact();
}
//calling up and animating array
...
//////////////////////////////////////////////////////////////////////////////  
 ...
 text("      " + scoreTotal, indent, 300);
 text("      " + 1050, indent, 355);
 text("      " + 896, indent, 410);
 text("      " + 742, indent, 465);
 text("      " + 527, indent, 515);
...
void typewriteText2() {
  ...
  smooth();
   ...
   if(counter3 < text3.length()){
    counter3++;
    text(text3.substring(0, counter3), 600, 775, width, height);
   ...
   if(counter3>=text3.length() && counter4 < text4.length()){
    counter4++;
    text(text4.substring(0, counter4), 600, 825, width, height);
